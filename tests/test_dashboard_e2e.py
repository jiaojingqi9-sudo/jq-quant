...
# 首页约 0.5 秒，但股票页要加载实盘账本与行情，实测约 55 秒（2026-07-30 在真实
# 数据目录下测得；用改动前后的代码各测一次，分别是 54.7s 与 54.8s，说明这是页面
# 本身的固有耗时，不是某次改动引入的）。原来的 30 秒上限在数据量变大后已经不够，
# 表现为「运行超时」而非断言失败，很容易被误读成路由坏了。
#
# 这个耗时本身值得优化，但那是另一件事；测试不应该用一个必然超时的上限。
TIMEOUT = 150
