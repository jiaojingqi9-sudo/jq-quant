...
class PipelineIntegrationTest(unittest.TestCase):
    def test_pipeline_produces_ranked_events_and_instruments(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("ai_judgement", report_payload)
            self.assertIn("events", report_payload["ai_judgement"])
            self.assertIn("instruments", report_payload["ai_judgement"])
            ...
            self.assertIn('id="aiWorkspace"', dashboard_html)
            self.assertIn('id="techWorkspace"', dashboard_html)
            self.assertIn('id="frontierWorkspace"', dashboard_html)
            self.assertIn('id="aiJudgementList"', dashboard_html)
            self.assertEqual(dashboard_html.count('id="aiJudgementList"'), 1)
            self.assertIn('ai: "AI判断"', dashboard_html)
            self.assertIn('const views = ["core", "ai", "tech", "frontier"]', dashboard_html)
            self.assertIn("function renderAiJudgement()", dashboard_html)
            ...
            self.assertIn('id="lexiconAiReviewButton"', dashboard_html)
            self.assertIn('id="aiAskInput"', dashboard_html)
            self.assertIn("data-drag-news-id", dashboard_html)
            self.assertIn("function attachNewsDragHandlers", dashboard_html)
            self.assertIn("来自看板拖入", dashboard_html)
            self.assertIn('/api/ai/analyze-news', dashboard_html)
